...
base.cube_add(
    target=antenna,
    name="antenna_base",
    scale=(radius_antenn1 * 2, radius_antenn1, frame_thickness + 1),
    location=(0, -3, -5.77),
...
base.cube_clear(
    target=frame,
    name="main-cut",
    scale=(plate_width * 2, plate_width * 2, 10),
    location=(0, 0, -10 + frame_thickness / 1.2),
)
...
base.frame_add(
    target=frame, name="frame_right", inner=erls, thickness=frame_thickness, location=(x, y, z)
)
base.frame_add(
    target=frame, name="frame_left", inner=erls, thickness=frame_thickness, location=(-x, y, z)
)
